...
async blockInterpretation() {
  ...
  try {
    ...
    const payload = {
      ...
      extraSystemPrompt: "Составь итоговое толкование этого блока сновидения (3–6 предложений), используя rolling summary, последние сообщения и сам текст блока. Свяжи общие мотивы: части тела, числа/цифры, запретные импульсы, детские переживания. Не повторяй и не цитируй текст блока, не пересказывай его дословно. Не задавай вопросов. Интерпретируй образы, чувства и скрытые мотивы, которые могут стоять за этим фрагментом сна. Избегай любых психоаналитических понятий и специальных терминов. Выведи только чистый текст без заголовков, без кода и без тегов."
    };
